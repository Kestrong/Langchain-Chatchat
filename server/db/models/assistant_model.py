...
class AssistantModel(Base):
    ...
    force_feedback = Column(String(4), default='0BF', server_default='0BF', comment='是否强制点赞后才能继续对话')
    state = Column(String(4), default='0BT', server_default='0BT', comment='状态：0BF禁用，0BT启用')
    history_len = Column(Integer, default=HISTORY_LEN, server_default=f'{HISTORY_LEN}', comment='历史对话轮数')
    top_k = Column(Integer, default=-1, server_default='-1', comment='知识库匹配条数')
    score_threshold = Column(Float, default=-1.0, server_default='-1.0', comment='知识库匹配阈值')
