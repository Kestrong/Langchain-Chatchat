...
class AssistantModel(Base):
    ...
    def __repr__(self):
        return f"<assistant(id='{self.id}', name='{self.name}', avatar='{self.avatar}', prompt='{self.prompt}', model_name='{self.model_name}', prologue='{self.prologue}', knowledge_base_ids='{self.knowledge_base_ids}', extra='{self.extra}', model_config='{self.model_config}', create_time='{self.create_time}', create_by='{self.create_by}', sort_id='{self.sort_id}')>"
